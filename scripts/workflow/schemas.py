"""Versioned Workflow JSON document schemas.
...
def _validate_metadata_source(data: Mapping[str, Any], schema: SchemaKind) -> None:
    ...
    if not isinstance(source, Mapping):
        raise _field(schema, "source", "must be an object")

    mode = source.get("storage_mode")
    if not isinstance(mode, str) or not mode.strip():
        raise _field(schema, "source.storage_mode", "must be a non-empty string")
    ...
    if not isinstance(filename, str) or not filename.strip():
        raise _field(schema, "source.filename", "must be a non-empty string")
    ...
    if type(size) is not int:
        raise _field(schema, "source.size_bytes", "must be an integer")
    if size < 0:
        raise _field(schema, "source.size_bytes", "must be >= 0")
...
def _validate_manifest_source_extension(data: Mapping[str, Any], schema: SchemaKind) -> None:
    ...
    if has_mode != has_size:
        ...
        raise _field(schema, missing, "is required with explicit source manifest identity")
    if not has_mode:
        return
    ...
    if type(size) is not int:
        raise _field(schema, "source_size_bytes", "must be an integer")
    if size < 0:
        raise _field(schema, "source_size_bytes", "must be >= 0")
...
def _validate_shared_state_snapshot(
    ...
    *,
    ...
    if not isinstance(snapshot, Mapping):
        raise _field(schema, path, "must be an object")
...
def _validate_translation_acceptance(
    ...
    *,
    ...
    if type(version) is not int or version != 1:
        raise _field(schema, f"{prefix}.schema_version", "must equal 1")
    ...
    _validate_hex_id(claim_id, schema, f"{prefix}.claim_id")
    for key in (
        "claim_revision",
        "session_id",
        "base_revision",
    ...
    if evidence.get("role") != "translator":
        raise _field(schema, f"{prefix}.role", "must equal translator")
    ...
    for key in ("source_sha256", "translation_sha256"):
        ...
        _validate_sha256(digest, schema, f"{prefix}.{key}")

    accepted_at = _require_nonempty_string(
        evidence,
        "accepted_at",
        schema,
        path=f"{prefix}.accepted_at",
    )
    _parse_utc_timestamp(accepted_at, schema, f"{prefix}.accepted_at")
