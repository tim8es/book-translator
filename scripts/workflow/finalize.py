"""Atomic, crash-recoverable Workflow finalization core."""
...
class FinalizationError(RuntimeError):
    """Base error for Workflow finalization."""
...
def render_state_markdown(snapshot: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Deterministic projection of authoritative Workflow completion state.",
        "",
        "## Revisions",
        "",
...
class FinalizationManager:
        ...
        try:
            ...
            if marker is None:
                ...
                try:
                    ...
                    marker = LoadedDocument(data=data, version=version)
                    created = True
    ...
    def _promote_marker(self, marker: LoadedDocument, progress_revision: str) -> LoadedDocument:
        ...
        return LoadedDocument(data=updated, version=version)
