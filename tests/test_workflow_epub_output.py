...
from workflow.reviews import ReviewResolution

try:
    epub_output = importlib.import_module("workflow.epub_output")
...
class WorkflowEpubOutputIdentityTests(unittest.TestCase):
    def setUp(self):
        self.metadata = {
            "schema_version": 1,
            "title": "Example Book",
    ...
    def api(self, name):
        self.assertIsNotNone(epub_output, "workflow.epub_output is not implemented")
...
class WorkflowOutputManifestTests(unittest.TestCase):
    def api(self, name):
        self.assertIsNotNone(epub_output, "workflow.epub_output is not implemented")
        value = getattr(epub_output, name, None)
        self.assertIsNotNone(value, f"{name} is not implemented")
